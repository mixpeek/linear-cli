...
const EditIssue: React.FC<EditIssueProps> = ({ issue, onBack, onUpdate }) => {
  ...
  const fields: Array<{
    ...
    { key: 'status', label: 'Status', type: 'select', help: '← → to change', selectionOptions: options.states },
    { key: 'priority', label: 'Priority', type: 'select', help: '← → to change', options: [0, 1, 2, 3, 4] },
    { key: 'assignee', label: 'Assignee', type: 'select', help: 'Type to search, Enter to select', selectionOptions: options.users },
    { key: 'team', label: 'Team', type: 'select', help: '← → to change', selectionOptions: options.teams },
    { key: 'project', label: 'Project', type: 'select', help: '← → to change', selectionOptions: options.projects }
  ];
  ...
  const handleSelectionChange = (
    ...
    if (field.selectionOptions) {
      ...
      const currentIndex = options.findIndex(opt => 
        String(opt) === String(currentValue)
      );
  ...
  return (
    <Box flexDirection="column">
      <Box marginBottom={1}>
        <Text>Editing Issue: {issue.identifier}</Text>
      </Box>
